...
void ATLASsub_AddV2()
{
    ...
    TFile* _file[20];
    TFile* _file_low;
    _file_low = TFile::Open("../MB0_20/ppCorr_pt033.root");
    ...
    _file_low->GetObject("signal",signal_low);
    _file_low->GetObject("background",background_low);
    _file_low->GetObject("mult",mult_low);
    ...
    double V2low = fit2->GetParameter(2)+0.0025;
    ...
    for(int i=0;i<8;i++){
        ...
        V2true[i] = Ntrk[i]*3e-05;
        //if(i==0) V2true[i] = 0;
        
        TF1* f1 = new TF1("f1",Form("%f*2.0*%f*cos(2.0*x)",Nassoc[i],0.0025),-0.5*TMath::Pi(),1.5*TMath::Pi());
        //TF1* f1 = new TF1("f1",Form("%f*2.0*%f*cos(2.0*x)",Nassoc[i],0.0025),-0.5*TMath::Pi(),1.5*TMath::Pi());
        alrs[i]->Add(f1);
    ...
    for(int i=0;i<8;i++)
    {
        ...
        cout<<"input V2: "<<Ntrk[i]*1.57142857142857237e-05<<endl;
        //cout<<"input V2: "<<0.0025<<endl;
    ...
    TGraphErrors* V2plottrue = new TGraphErrors(8,Ntrk,V2true,0,0);
    ...
    V2plottrue->SetName("hadronV2trueFlow");
    ...
    TFile ofile("v2sub020_ATLAS_pt033_addLinearV2_slope3e05.root","RECREATE");
    V2plot->Write();
    V2subplot->Write();
    v2plot->Write();
    v2subplot->Write();
